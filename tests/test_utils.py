...
def mock_asset_manager():
    ...
    def mock_get_rect(**kwargs):
        ...
        if "topleft" in kwargs:
            ...
            return pygame.Rect(
                (kwargs["center"][0] - width // 2,
                 kwargs["center"][1] - height // 2),
                (width, height),
            )
    ...
    def create_mock_surface(width=32, height=32):
        ...
        mock_surface.get_rect = MagicMock(
            side_effect=lambda **kwargs: mock_get_rect(
    ...
    manager.images = {
        ...
        "continue_button": create_mock_surface(200, 50),  # Corrected key
        "settings_button": create_mock_surface(200, 50),  # Corrected key
    }

    # Digit images for UI
    for i in range(10):
        manager.images[f"digit_{i}"] = create_mock_surface(
            10, 15)  # Example digit size
...
def mock_pygame_init_and_display(mocker):
    ...
    if not pygame.get_init():
        # Ensure pygame is initialized for Surface creation, etc.
        pygame.init()

    mocker.patch("pygame.display.set_mode",
                 return_value=MagicMock(spec=pygame.Surface))
    mocker.patch(
        "pygame.display.get_surface", return_value=MagicMock(spec=pygame.Surface)
    )

    # Mock font rendering
    mock_font_render_surface = MagicMock(spec=pygame.Surface)
    mock_font_render_surface.get_rect = MagicMock(
        return_value=pygame.Rect(0, 0, 50, 20)
    )
    mock_font_instance = MagicMock()
    mock_font_instance.render = MagicMock(
        return_value=mock_font_render_surface)
