from unittest.mock import DEFAULT as MOCK_DEFAULT  # Import DEFAULT
from unittest.mock import MagicMock, patch

import pygame  # Needed for pygame.Surface, pygame.error, pygame.font, pygame.mixer types
import pytest

from asset_manager import (  # The class we're testing and DummySound
    AssetManager,
    DummySound,
)
from config import (
    # Import for type hint or comparison
    DEFAULT_PLACEHOLDER_SIZE as REAL_DEFAULT_PLACEHOLDER_SIZE,
)
from config import IMAGE_ASSET_CONFIG as REAL_IMAGE_ASSET_CONFIG  # To see its structure
from config import SOUND_ASSET_CONFIG as REAL_SOUND_ASSET_CONFIG
...
class TestAssetManagerImageLoading:
    ...
    @patch(
        "pygame.image.load", side_effect=pygame.error("Failed to load image for test")
    )
    @patch(
        "asset_manager.get_asset_path", return_value="dummy/failing/path"
    )  # Target where it's used
    @patch("pygame.Surface")
    def test_load_image_failure_creates_placeholder_with_correct_size(
        self,
        mock_pygame_surface_constructor,
        mock_am_get_asset_path,
        mock_pygame_load,
        am,
        mocker,
        caplog,
    ):  # Renamed mock variable
        ...
        # Specific default for this test
        test_default_placeholder_size = (70, 30)
        ...
        mocker.patch("asset_manager.IMAGE_ASSET_CONFIG",
                     mock_image_config_dict)
        mocker.patch(
            "asset_manager.DEFAULT_PLACEHOLDER_SIZE", test_default_placeholder_size
        )
        ...
        def surface_side_effect(size_arg):
            ...
            mock_surf.get_rect = MagicMock(
                return_value=pygame.Rect(0, 0, size_arg[0], size_arg[1])
            )
        ...
        assert hinted_size in created_surface_sizes, (
            f"pygame.Surface not called with hinted size {hinted_size}"
        )
        ...
        assert (
            f"Placeholder font not available for asset '{hinted_asset_key}'"
            in caplog.text
        )
        assert (
            f"Placeholder font not available for asset '{no_hint_asset_key}'"
            in caplog.text
        )
...
class TestAssetManagerSoundLoading:
    # Removed @patch('builtins.print')
    # Ensure no images are processed
    @patch("asset_manager.IMAGE_ASSET_CONFIG", {})
    def test_load_sound_when_mixer_not_initialized(
        self, am, mocker, caplog
    ):  # Added caplog, removed mock_print
        ...
        expected_log_message = (
            f"Pygame mixer not initialized. Using dummy sound for '{asset_key_to_test}'"
        )
    ...
    # Ensure no images are processed
    @patch("asset_manager.IMAGE_ASSET_CONFIG", {})
    def test_load_sound_failure_logs_warning_and_uses_dummy(
        self, mock_am_get_asset_path, mock_pygame_sound, am, mocker, caplog
    ):  # Renamed mock variable
        ...
        original_asset_path = REAL_SOUND_ASSET_CONFIG[
            asset_key_to_test
        ...
        mock_am_get_asset_path.assert_any_call(original_asset_path)
        mock_pygame_sound.assert_any_call(
            f"resolved/dummy/{original_asset_path}")
        ...
        )  # Check if the fragment is in the captured log text

        assert asset_key_to_test in am.sounds  # Key should now be present
        assert isinstance(
            am.sounds[asset_key_to_test], DummySound
        )  # Value should be a DummySound instance
...
class TestAssetManagerFontInitialization:
    ...
    def test_font_initialization_success(self, mock_builtin_hasattr, mocker):
        ...
        if font_system_works:
            ...
            assert isinstance(
                asset_manager_instance.placeholder_font, pygame.font.FontType
            )
        else:
    ...
    @patch("builtins.hasattr")
    # Removed @patch('builtins.print')
    def test_font_initialization_failure_via_hook(
        self, mock_builtin_hasattr, caplog
    ):  # Added caplog, removed mock_print
    ...
    def test_font_initialization_fails_if_pygame_font_is_none(
        self, mocker, caplog
    ):  # Added caplog, removed mock_print
        ...
        assert expected_warning_fragment in caplog.text
        assert (
            "'NoneType' object has no attribute" in caplog.text
        )  # More specific check for the NoneType error

    # Removed @patch('builtins.print')
    @patch("builtins.hasattr")  # Patch builtins.hasattr for this specific test
    def test_font_module_truly_missing(
        self, mock_builtin_hasattr, mocker, caplog
    ):  # Added caplog
        ...
        assert asset_manager_instance.placeholder_font is None
        expected_warning = (
            "Pygame font module not available. Placeholders will not have text."
        )
