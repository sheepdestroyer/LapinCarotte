...
class TestCollectibleInitialization:
    ...
    def test_different_item_type_and_scale(self, item_image):
        ...
        expected_width = int(item_image.get_width() * custom_scale) # 30 * 0.25 = 7.5 -> 7
        expected_height = int(item_image.get_height() * custom_scale) # 40 * 0.25 = 10
    ...
    def test_pygame_transform_scale_called_correctly(self, mock_transform_scale, item_image):
        ...
        mock_scaled_surface.get_width.return_value = int(item_image.get_width() * ITEM_SCALE)
        mock_scaled_surface.get_height.return_value = int(item_image.get_height() * ITEM_SCALE)
        ...
        def mock_get_rect_for_scaled(**kwargs):
            if 'center' in kwargs:
                return pygame.Rect(
                    ...
                    kwargs['center'][1] - mock_scaled_surface.get_height() // 2,
                    mock_scaled_surface.get_width(),
                    mock_scaled_surface.get_height()
                )
            return pygame.Rect(0,0, mock_scaled_surface.get_width(), mock_scaled_surface.get_height())

        mock_scaled_surface.get_rect = MagicMock(side_effect=mock_get_rect_for_scaled)
        ...
        assert collectible.original_image is mock_scaled_surface # Due to GameObject constructor
