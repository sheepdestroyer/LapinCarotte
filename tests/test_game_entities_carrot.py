...
class TestCarrotInitialization:
    def test_initial_values(self, carrot_instance, carrot_image):
        ...
        assert (
            round(carrot_instance.direction.length(), 5) == 1.0
...
class TestCarrotRespawn:
    def test_respawn_resets_state(self, carrot_instance, carrot_image):
        ...
        with patch(
            "random.uniform", MagicMock(side_effect=[0.6, 0.8])
        ) as mock_rand_uniform:  # yields Vector2(0.6,0.8).normalize()
...
class TestCarrotUpdateMovement:  # Testing Carrot.update, not GameState's carrot logic
    @patch(
        "random.uniform", MagicMock(return_value=0.0)
    )  # Disable random wandering for predictable tests
    def test_movement_when_player_far(self, carrot_instance):
        ...
        assert (
            carrot_instance.rect.y == initial_pos.y
        )  # No vertical movement if direction is (1,0)
    ...
    def test_movement_when_player_close_moves_away(self, carrot_instance, carrot_image):
        ...
        player_rect.center = (
            carrot_instance.rect.centerx - 20,
            carrot_instance.rect.centery,
        )  # Player to the left
    ...
    def test_stays_within_world_bounds(self, carrot_instance):
        player_rect = pygame.Rect(
            0, 0, 32, 32
        )  # Player position doesn't matter for this boundary test
        ...
        carrot_instance.rect.topleft = (
            WORLD_SIZE[0] - carrot_instance.rect.width,
            WORLD_SIZE[1] - carrot_instance.rect.height,
        )
        assert carrot_instance.rect.bottomright == (
            WORLD_SIZE[0],
            WORLD_SIZE[1],
        )  # Pre-check
    ...
    def test_speed_multiplier_increases_speed(self, carrot_instance):
        ...
        carrot_instance.direction = pygame.math.Vector2(
            1, 0)  # Known direction
        ...
        player_rect_far.center = (
            carrot_instance.rect.centerx + CARROT_DETECTION_RADIUS,
            carrot_instance.rect.centery,
        )
        ...
        player_rect_at_detection_radius.center = (
            carrot_instance.rect.centerx - CARROT_DETECTION_RADIUS,
            carrot_instance.rect.centery,
        )  # dist = CARROT_DETECTION_RADIUS
        ...
        assert moved_close_x > moved_far_x * (MAX_SPEED_MULTIPLIER - 1), (
            "Movement with high multiplier not sufficiently larger"
        )
