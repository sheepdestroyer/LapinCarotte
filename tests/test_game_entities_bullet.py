...
class TestBulletInitialization:
    def test_initial_values_horizontal(self, bullet_instance, bullet_image):
        ...
        assert bullet_instance.rect.topleft == (
            50,
            50,
        )  # Rect is still based on x,y topleft
...
class TestBulletUpdate:
    def test_update_moves_rect(self, bullet_instance):
        ...
        vel_x, vel_y = (
            bullet_instance.velocity
        )  # This velocity is now correctly calculated from (50,50)
...
class TestBulletRotatedImage:
    ...
    def test_rotated_image_property_calls_transform_rotate(
        self, mock_rotate, bullet_instance, bullet_image
    ):
        ...
        assert (
            rotated_img is mock_rotate.return_value
        )  # Ensure the property returns what rotate returns
