...
def mock_pygame_modules(monkeypatch):
    ...
    mock_screen_surface.get_size.return_value = (
        1920,
        1080,
    ...
    monkeypatch.setattr(
        pygame.display, "set_mode", MagicMock(return_value=mock_screen_surface)
    )
    monkeypatch.setattr(pygame.display, "set_icon", MagicMock())
    monkeypatch.setattr(
        pygame.display, "flip", MagicMock()
    )  # Nécessaire si main_loop est appelé
    ...
    # Mock the image module itself
    monkeypatch.setattr(pygame, "image", MagicMock())
    monkeypatch.setattr(
        pygame.image, "load", MagicMock(return_value=mock_image_surface)
    )
    ...
    # This blit can accept MagicMocks
    mock_created_surface_instance.blit = MagicMock()
    mock_created_surface_instance.convert_alpha.return_value = (
        mock_created_surface_instance
    )
    mock_created_surface_instance.get_rect.return_value = real_pygame_rect(
        0, 0, 100, 100
    )  # Example default
    ...
    monkeypatch.setattr(
        pygame, "Surface", MagicMock(
            return_value=mock_created_surface_instance)
    )
    ...
    monkeypatch.setattr(pygame.mixer.music, "get_busy",
    ...
    # Use real_pygame_rect for spec
    mock_start_rect = MagicMock(spec=real_pygame_rect)
    mock_start_rect.width = 100
    mock_start_rect.height = 50

    # Use real_pygame_rect for spec
    mock_exit_rect = MagicMock(spec=real_pygame_rect)
    mock_exit_rect.width = 100
    mock_exit_rect.height = 50

    mock_restart_rect = MagicMock(
        spec=real_pygame_rect
    )  # Use real_pygame_rect for spec
    ...
    mock_start_screen_surface = MagicMock(
        spec=real_pygame_surface
    )  # Use real_pygame_surface for spec
    ...
    mock_start_screen_surface.get_rect.return_value = real_pygame_rect(
        0, 0, 1920, 1080)
    ...
    start_button_mock_img.get_width.return_value = (
        mock_start_rect.width
    )  # Consistent width
    start_button_mock_img.get_height.return_value = (
        mock_start_rect.height
    )  # Consistent height
    ...
    mock_asset_manager_instance.images["continue_button"] = (
        mock_image_surface  # Corrected key
    )
    mock_asset_manager_instance.images["settings_button"] = (
        mock_image_surface  # Corrected key
    )
    ...
    monkeypatch.setattr(
        asset_manager,
        "AssetManager",
        MagicMock(return_value=mock_asset_manager_instance),
    )
    ...
    default_args = argparse.Namespace(
        cli=False, debug=True
    )  # Default to non-CLI, debug True for tests
    ...
    _mock_gs.player.health = (
        config.START_HEALTH
    )  # Access START_HEALTH from main's imported config
    # Ensure the mock GameState uses the mock AssetManager
    _mock_gs.asset_manager = mock_asset_manager_instance
    ...
    def mock_reset_side_effect():
        ...
        _mock_gs.player.health = (
            config.START_HEALTH
        )  # Use main.START_HEALTH for consistency
...
def test_game_initialization_no_errors():
    ...
    try:
        ...
        with (
            patch("pygame.quit"),
            patch("sys.exit"),
        ):  # Empêche la fermeture de Pygame/sys
...
def test_start_game_functionality(mock_pygame_modules):
    ...
    pygame.mixer.music.load.assert_called_with(
        "dummy_lambda_path")  # MUSIC_GAME
...
def test_reset_game_functionality(mock_pygame_modules):
    ...
    assert (
        main.game_state.player.health == config.START_HEALTH
    )  # Vérifier une valeur spécifique réinitialisée
    main.asset_manager.sounds["press_start"].play.assert_called_once()
