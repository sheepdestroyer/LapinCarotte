...
def handle_player_death():
    ...
    *Utilise le 'current_time' global pour synchroniser l'effet de mort.*
    ...
    if not game_state.game_over and not game_state.player.death_effect_active:
        ...
        logging.info(
            "Player death sequence initiated. / Séquence de mort du joueur initiée."
        )
    ...
    if not args.cli and pygame.mixer.get_init():
        try:
            ...
            logging.exception(
                f"Could not play player death sound: {e} / Impossible de jouer le son de mort du joueur : {e}"
            )
    elif args.cli:
        logging.info(
            "Player has died (CLI mode). / Le joueur est mort (mode CLI).")
...
def _play_game_music_and_sound(sound_to_play=None):
    ...
    *Args:*
    ...
    """
    if args.cli or not pygame.mixer.get_init():
        return
    try:
        pygame.mixer.music.stop()
        ...
        logging.exception(
            f"Could not load or play game music/sound: {e} / Impossible de charger ou de jouer la musique/le son du jeu : {e}"
        )
...
def start_game():
    ...
    *Fonction de rappel pour démarrer le jeu. Met game_state.started à True et joue le son de démarrage.*
    ...
    if not args.cli:
        ...
        logging.info("Game started (CLI). / Jeu démarré (CLI).")
    logging.debug(
        f"game_state.started set to {game_state.started} / game_state.started défini sur {game_state.started}"
    )
...
def reset_game():
    ...
    *Fonction de rappel pour réinitialiser l'état du jeu pour une nouvelle partie.*
    ...
    if not args.cli:
        ...
        logging.info("Game reset (CLI). / Jeu réinitialisé (CLI).")
    logging.debug(
        "Game state reset by reset_game function. / État du jeu réinitialisé par la fonction reset_game."
    )
...
def quit_game():
    ...
    *Fonction de rappel pour quitter le jeu. Met l'indicateur global 'running' à False.*
    ...
    running = False
    if args.cli:
        logging.info("Exiting game (CLI). / Sortie du jeu (CLI).")
    else:
        logging.info("Exiting game (GUI). / Sortie du jeu (GUI).")
    logging.debug(
        "Global 'running' flag set to False. / Indicateur global 'running' défini sur False."
    )
...
def resume_game_callback():
    ...
    *Fonction de rappel pour reprendre le jeu s'il était en pause.*
    """
    logging.debug(
        "resume_game_callback called. / resume_game_callback appelé.")
    if game_state.paused:
        logging.debug(
            "Game was paused, calling game_state.resume_game() / Le jeu était en pause, appel de game_state.resume_game()"
        )
        game_state.resume_game()
        if args.cli:
            logging.info(
                "Game resumed via callback (CLI). / Jeu repris via rappel (CLI)."
            )
    else:
        logging.debug(
            "Game was not paused, resume_game_callback did nothing. / Le jeu n'était pas en pause, resume_game_callback n'a rien fait."
        )
...
def open_settings_callback():
    ...
    *Rappel pour le bouton des paramètres. Actuellement un placeholder.*
    """
    logging.debug(
        "open_settings_callback called. / open_settings_callback appelé.")
    logging.info(
        "Settings button clicked - Feature not implemented yet. / Bouton Paramètres cliqué - Fonctionnalité non encore implémentée."
    )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="LapinCarotte - A game about a rabbit fighting vampire carrots. / *Un jeu sur un lapin combattant des carottes vampires.*"
    )
    parser.add_argument(
        "--cli",
        action="store_true",
        help="Run the game in Command Line Interface mode (no graphics). / *Exécuter le jeu en mode Interface en Ligne de Commande (sans graphismes).*",
    )
    parser.add_argument(
        "-d",
        "--debug",
        action="store_true",
        help="Enable debug logging output. / *Activer la sortie de journalisation de débogage.*",
    )
...
def setup_logging(args):
    ...
    if args.debug:
        logging.info(
            "Debug logging enabled. / Journalisation de débogage activée.")
...
def load_game_assets(args, asset_manager, screen_width, screen_height):
    ...
    if not args.cli:
        if "icon" in asset_manager.images and hasattr(
            asset_manager.images["icon"], "get_rect"
        ):
        ...
        if sys.platform == "win32":
            ...
            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(
                "LapinCarotte.LapinCarotte.Game.1.0"
            )
        ...
        if (
            start_screen_image
            and hasattr(start_screen_image, "get_width")
            and hasattr(start_screen_image, "get_height")
        ):
        ...
        if grass_image and hasattr(grass_image, "get_size"):
            grass_background = pygame.Surface(
                config.WORLD_SIZE, pygame.SRCALPHA)
            ...
            if _grass_w > 0 and _grass_h > 0:
                ...
                logging.warning(
                    "Grass asset has invalid dimensions, cannot tile background."
                )
...
def create_buttons(args, screen_width, screen_height, assets, callbacks):
    ...
    if not args.cli:
        ...
        if _restart_img_temp and hasattr(_restart_img_temp, "get_rect"):
            restart_button_rect = _restart_img_temp.get_rect()

        _exit_img_temp = assets.get("exit_button_img")
        if _exit_img_temp and hasattr(_exit_img_temp, "get_rect"):
            exit_button_rect = _exit_img_temp.get_rect()

        _start_img_temp = assets.get("start_button_img")
        if _start_img_temp and hasattr(_start_img_temp, "get_rect"):
            start_button_rect = _start_img_temp.get_rect()

        _continue_img_temp = assets.get("continue_button_img")
        if _continue_img_temp and hasattr(_continue_img_temp, "get_rect"):
            continue_button_rect = _continue_img_temp.get_rect()

        _settings_img_temp = assets.get("settings_button_img")
        if _settings_img_temp and hasattr(_settings_img_temp, "get_rect"):
            settings_button_rect = _settings_img_temp.get_rect()
    ...
    start_screen_buttons = [
        start_button_start_screen, exit_button_start_screen]
    ...
    if screen_width > 0:
        game_over_x_restart = (
            screen_width // 2 -
            (_restart_w + config.BUTTON_SPACING + _exit_w) // 2
        )
    ...
    game_over_buttons = [restart_button_game_over_screen,
                         exit_button_game_over_screen]
    ...
    if screen_height > 0:
        total_pause_buttons_height = (
            continue_button_rect.height
            + config.BUTTON_SPACING
            + settings_button_rect.height
        )
        pause_y_continue = screen_height // 2 - total_pause_buttons_height // 2
        pause_y_settings = (
            pause_y_continue + continue_button_rect.height + config.BUTTON_SPACING
        )
    else:
        pause_y_continue = 0
        pause_y_settings = 0
    ...
    pause_screen_buttons = [
        continue_button_pause_screen, settings_button_pause_screen]
...
def run_gui_mode():
    ...
    *Cette fonction est appelée de manière répétée par main_loop lorsqu'on n'est pas en mode CLI.*
    """
    logging.debug(
        "run_gui_mode: Frame processing started. / run_gui_mode : Traitement de la frame démarré."
    )
    global \
        running, \
        current_time, \
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            logging.info(
                "QUIT event received, shutting down. / Événement QUIT reçu, fermeture en cours."
            )
            running = False
            return
        ...
        if not game_state.started:
            ...
            if event.type == pygame.KEYDOWN:
                if (
                    ...
                    and not game_state.player.death_effect_active
                ):
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if (
                    event.button == 3
                    and not game_state.player.death_effect_active
                    and game_state.player.garlic_count > 0
                    and game_state.garlic_shot is None
                ):
                    ...
                    dx_norm, dy_norm = (
                        dx / dist, dy / dist) if dist > 0 else (0, 1)
                    ...
                    logging.debug(
                        f"Garlic shot initiated towards ({world_mouse_x},{world_mouse_y}) with angle {angle:.2f} / Tir d'ail initié vers ({world_mouse_x},{world_mouse_y}) avec un angle de {angle:.2f}"
                    )
        else:
    ...
    if not game_state.started:
        ...
        if screen and game_over_image_ui and hasattr(game_over_image_ui, "get_width"):
            game_over_x_pos = (
                screen_width - game_over_image_ui.get_width()) / 2
            game_over_y_pos = (
                screen_height - game_over_image_ui.get_height()) / 2
        ...
        if game_state.player.rect.left < game_state.scroll[0] + scroll_margin_x:
            game_state.scroll[0] = max(
                0, game_state.player.rect.left - scroll_margin_x)
        elif (
            game_state.player.rect.right
            > game_state.scroll[0] + screen_width - scroll_margin_x
        ):
            game_state.scroll[0] = min(
                game_state.world_size[0] - screen_width,
                game_state.player.rect.right - screen_width + scroll_margin_x,
            )

        if game_state.player.rect.top < game_state.scroll[1] + scroll_margin_y:
            game_state.scroll[1] = max(
                0, game_state.player.rect.top - scroll_margin_y)
        elif (
            game_state.player.rect.bottom
            > game_state.scroll[1] + screen_height - scroll_margin_y
        ):
            game_state.scroll[1] = min(
                game_state.world_size[1] - screen_height,
                game_state.player.rect.bottom - screen_height + scroll_margin_y,
            )

        game_state.scroll[0] = max(
            0, min(game_state.scroll[0],
                   game_state.world_size[0] - screen_width)
        )
        game_state.scroll[1] = max(
            0, min(game_state.scroll[1],
                   game_state.world_size[1] - screen_height)
        )
        ...
        try:
            ...
            if screen and grass_background and hasattr(grass_background, "get_width"):
                screen.blit(
                    grass_background, (-game_state.scroll[0], -
                                       game_state.scroll[1])
                )

            for carrot_enemy in game_state.carrots:
                if carrot_enemy.active and screen:
                    carrot_enemy.draw(screen, game_state.scroll)

            player_pos_on_screen = (
                game_state.player.rect.x - game_state.scroll[0],
                game_state.player.rect.y - game_state.scroll[1],
            )
            if game_state.player.death_effect_active:
                if (
                    int(
                        (current_time - game_state.player.death_effect_start_time) / 0.1
                    )
                    % 2
                    == 0
                ):
                    if (
                        screen
                        and game_state.player.original_image
                        and hasattr(game_state.player.original_image, "copy")
                    ):
                ...
                game_state.player.invincible
                and int(current_time * config.PLAYER_INVINCIBILITY_FLASH_FREQUENCY) % 2
                == 1
            ):
                pass
            else:
                if screen:
                    game_state.player.draw(screen, game_state.scroll)
            ...
            if game_state.garlic_shot and game_state.garlic_shot["active"]:
                if screen and garlic_image and hasattr(garlic_image, "get_rect"):
                    rotated_garlic = pygame.transform.rotate(
                        garlic_image, game_state.garlic_shot["rotation_angle"]
                    )
                    rotated_rect = rotated_garlic.get_rect(
                        center=(
                            game_state.garlic_shot["x"],
                            game_state.garlic_shot["y"],
                        )
                    )
                    screen.blit(
                        rotated_garlic,
                        (
                            rotated_rect.x - game_state.scroll[0],
            ...
            if screen:
                game_state.vampire.draw(
                    screen, game_state.scroll, current_time)

            if screen and hp_image_ui and garlic_image:
                game_state.player.draw_ui(
                    screen, hp_image_ui, garlic_image, config.MAX_GARLIC
                )

            if (
                game_state.player.health_changed
                or game_state.player.garlic_changed
                or game_state.player.juice_changed
            ):
                logging.debug(
            ...
            for item in game_state.items:
                if item.active and screen:
                    item.draw(screen, game_state.scroll)

        except Exception as e:
            logging.exception(
                f"ERROR during game logic/draw: {e} / ERREUR pendant la logique/le dessin du jeu : {e}"
            )
            running = False
            return
        ...
        if game_state.player.death_effect_active:
            time_elapsed_death = (
                current_time - game_state.player.death_effect_start_time
            )
            if time_elapsed_death >= config.PLAYER_DEATH_DURATION:
                game_state.game_over = True
                game_state.player.death_effect_active = False
                logging.info(
                    "Player death animation complete. Game Over. / Animation de mort du joueur terminée. Game Over."
                )
    else:
        if screen and game_over_image_ui and hasattr(game_over_image_ui, "get_width"):
            game_over_x_pos = (
        ...
        if pygame.mixer.get_init() and not pygame.mixer.music.get_busy():
            ...
            if music_path_game_over:
                try:
                    ...
                    logging.exception(
                        f"Error playing game over music: {e} / Erreur lors de la lecture de la musique de game over : {e}"
                    )
    ...
    if screen:
        ...
        if crosshair_img_ref and hasattr(crosshair_img_ref, "get_rect"):
            crosshair_rect_instance = crosshair_img_ref.get_rect(
    ...
    logging.debug(
        "run_gui_mode: Frame processing ended. / run_gui_mode : Traitement de la frame terminé."
    )
...
def run_cli_mode():
    ...
    *Présente des menus et des options textuels. La logique du jeu (comme les mises à jour d'entités) est minimale ou simulée en CLI.*
    ...
    logging.debug(
        "run_cli_mode: Iteration started. / run_cli_mode : Itération démarrée."
    )

    if not game_state.started:
        logging.debug(
            "run_cli_mode: Game not started, displaying start menu. / run_cli_mode : Jeu non démarré, affichage du menu de démarrage."
        )
        logging.info("\n--- LapinCarotte ---")
        logging.info(
            "Start Screen (CLI Mode) / *Écran de Démarrage (Mode CLI)*")
        ...
        try:
            ...
            logging.info(
                "EOF received on main menu, quitting. / EOF reçu dans le menu principal, fermeture."
            )
            quit_game()
            return
        ...
        if choice == "1":
            ...
            logging.warning(
                "Invalid choice on start screen. Please enter 1 or 2. / Choix invalide sur l'écran de démarrage. Veuillez entrer 1 ou 2."
            )

    elif game_state.paused:
        logging.debug(
            "run_cli_mode: Game paused, displaying pause menu. / run_cli_mode : Jeu en pause, affichage du menu pause."
        )
        logging.info("\n--- PAUSED (CLI Mode) / *PAUSE (Mode CLI)* ---")
        logging.info("1. Continue / *Continuer*")
        logging.info(
            "2. Settings (Not Implemented) / *Paramètres (Non implémenté)*")
        ...
        try:
            ...
            logging.info(
                "EOF received on pause menu, quitting. / EOF reçu dans le menu pause, fermeture."
            )
            quit_game()
            return
        ...
        if choice == "1":
            ...
            logging.warning(
                "Invalid choice on pause menu. / Choix invalide dans le menu pause."
            )

    elif game_state.game_over:
        logging.debug(
            "run_cli_mode: Game over, displaying game over menu. / run_cli_mode : Game over, affichage du menu game over."
        )
        logging.info("\n--- GAME OVER (CLI Mode) ---")
        ...
        try:
            ...
            logging.info(
                "EOF received on game over menu, quitting. / EOF reçu dans le menu game over, fermeture."
            )
            quit_game()
            return
        ...
        if choice == "1":
            ...
            logging.warning(
                "Invalid choice on game over menu. / Choix invalide dans le menu game over."
            )

    else:
        logging.debug(
            "run_cli_mode: Game active, displaying active game options. / run_cli_mode : Jeu actif, affichage des options du jeu actif."
        )
        logging.info(
            "\n--- Game Active (CLI Mode) / *Jeu Actif (Mode CLI)* ---")
        logging.info(
            f"Player HP: {game_state.player.health} (Note: Game logic not running in CLI yet / *Note : La logique du jeu ne tourne pas encore en CLI*)"
        )
        logging.info(
            "Options: (esc)ause, (q)uit, (d)ie (simulate death for testing) / *Options : (esc) Pause, (q)uitter, (d)écéder (simuler mort pour test)*"
        )
        ...
        try:
            ...
            logging.debug(
                f"CLI action received: '{cli_action}' / Action CLI reçue : '{cli_action}'"
            )
        except EOFError:
            logging.info(
                "EOF received during active game, quitting. / EOF reçu pendant le jeu actif, fermeture."
            )
            quit_game()
            return
        ...
        if cli_action == "esc":
            logging.debug(
                "CLI action 'esc' received, pausing game. / Action CLI 'esc' reçue, mise en pause du jeu."
            )
            game_state.pause_game()
        elif cli_action == "q":
            logging.debug(
                "CLI action 'q' received, quitting game. / Action CLI 'q' reçue, fermeture du jeu."
            )
            quit_game()
        elif cli_action == "d":
            logging.debug(
                "CLI action 'd' received, simulating player death. / Action CLI 'd' reçue, simulation de la mort du joueur."
            )
            game_state.player.health = 0
            game_state.game_over = True
            logging.info(
                "Simulating player death for CLI testing... Player HP set to 0, game_over set to True. / Simulation de la mort du joueur pour test CLI... PV Joueur mis à 0, game_over mis à True."
            )

    if running and args.cli:
        time.sleep(0.1)
    logging.debug(
        "run_cli_mode: Iteration ended. / run_cli_mode : Itération terminée.")
    ...
    *Continue tant que l'indicateur global 'running' est True.*
    ...
    while running:
        if not args.cli:
            ...
            run_cli_mode()
    logging.debug(
        "Main loop ended because 'running' is False. / Boucle principale terminée car 'running' est False."
    )
...
def main_entry_point():
    ...
    buttons = create_buttons(
        args, screen_width, screen_height, assets, callbacks)
    ...
    try:
        ...
        logging.info(
            "Application shutting down... / Fermeture de l'application...")
        if not args.cli and pygame.get_init():
            pygame.quit()
            logging.info(
                "Pygame quit successfully. / Pygame quitté avec succès.")
